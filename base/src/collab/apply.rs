    ...
    pub(crate) fn apply_patch(&mut self, patch: &Patch, ts: &Timestamp) {
        match patch {
            ...
            Patch::SetMergedRange {
                ...
                let at = sheet.merge_cells.iter().position(|r| r == range);
                match (merged, at) {
                    (true, None) => sheet.merge_cells.push(range.clone()),
                    (false, Some(at)) => {
                        sheet.merge_cells.remove(at);
                    }
                    _ => {}
                }
            }
    ...
    fn seed_sheet(&mut self, i: usize, content: &SheetContent, ts: &Timestamp) {
        ...
        for range in &content.merge_cells {
            sheet
                ...
                .registers
                .merges
                .insert(range.clone(), *ts);
            sheet.merge_cells.push(range.clone());
        }
        ...
        for (key, state) in &content.conditional_formatting {
            ...
            registers
                .cf
                .insert((key.clone(), CfPropKind::Rule), *ts);
            registers
                .cf
                .insert((key.clone(), CfPropKind::Ranges), *ts);
...
mod test {
    ...
    fn apply_registers() {
        ...
        assert_eq!(sheet.merge_cells, vec![merged.clone()]);
