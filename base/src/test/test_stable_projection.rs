...
use crate::types::{Col, Color, Comment, Dxf, Fill, Position, RangeRef, Row, Worksheet};
...
fn used_extent(ws: &Worksheet) -> (i32, i32) {
    ...
    let cf_ranges = ws.conditional_formatting.iter().flat_map(|cf| &cf.ranges);
    for range in ws.merge_cells.iter().chain(cf_ranges) {
...
pub(crate) fn stable_from_ordinal(ws: &Worksheet) -> Worksheet<Stable> {
    ...
    Worksheet {
        ...
        merge_cells: ws.merge_cells.iter().map(&range).collect(),
        comments: ws
            .comments
            .iter()
            .map(|c| Comment {
...
fn projection_identity() {
    ...
    model.evaluate();
    {
        ...
        ws.merge_cells.push(RangeRef::parse_a1("C3:D4").unwrap());
    ...
    assert_eq!(
        projected.merge_cells,
        vec![RangeRef::parse_a1("C2:D3").unwrap()]
    );
