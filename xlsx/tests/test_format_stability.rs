...
//! addressing (the build right before it, after `RangeRef` and upstream's worksheet `links` landed)
//! and is never regenerated from here — regenerating it would be exactly the regression this guards
//! against.
...
fn icalc_format_stability() {
    ...
    let merges = &model.workbook.worksheets[0].merge_cells;
    assert_eq!(merges.len(), 1);
    assert_eq!(merges[0].to_a1(), "C3:D4");
